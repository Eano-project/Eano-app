...
const cardBack = '../assets/back-card.png';
const specialEffects = {
  Square: 'Pick2', // Opponent draws 2 cards
  Circle: 'Freeze', // Skip opponent's turn
  Triangle: 'Wild', // Change suit
  Block: 'Reverse' // Reverse turn order (placeholder for future multiplayer)
};
...
function renderHand(hand, element, isAi = false) {
  ...
  hand.forEach((card, index) => {
    ...
    cardDiv.style.backgroundImage = isAi ? `url(${cardBack})` : 'none';
...
function applySpecialEffect(card) {
  ...
  switch (card.effect) {
    case 'Pick2':
      ...
      break;
    case 'Freeze':
      playerTurn = true; // Skip AI's turn
